...
| devpilot-litellm | LiteLLM 代理 | Anthropic↔OpenAI 协议翻译代理，Claude Code 经 http://litellm:4000 访问各平台 |
...
| 供应商管理 | 管理 AI 模型供应商配置，支持多供应商切换 | devpilot-litellm |
...
graph TB
    ...
    subgraph K8s 部署
        ...
        HM --> D3[Deployment: devpilot-claude]
...
1. **统一 bridge 网络**：所有容器加入 `devpilot-network` bridge 网络，通过容器名（`redis`、`openclaw`、`devpilot-claude`）进行 DNS 解析和内部通信。
...
- **devpilot-litellm**：本地 Anthropic ↔ OpenAI 翻译代理，容器内复刻 CC-Switch v3.16.1「Local Route / Claude Route」能力，对外暴露 `http://litellm:4000/v1/messages`，让 Claude Code 与全 5 平台均能正常对话。
...
| 安装组件 | `@anthropic-ai/claude-code@2.1.241` + devpilot-litellm (LiteLLM 1.82.6) | - |
| 容器名 | `devpilot-claude` | 固定名称 |
| 重启策略 | `unless-stopped` | - |
...
| 端口映射 | 容器内 `4000` (仅 docker network 内可达) | LiteLLM 代理端口 |
| 资源限制 | 768MB | - |
...
# devpilot-litellm 基础镜像由 python:3.13-slim 提供，pip 安装 litellm（见 dockerfiles/litellm/Dockerfile）
...
1. **devpilot-litellm 独立部署**：将 LiteLLM（CC-Switch 本地路由的底层引擎）独立部署为 devpilot-litellm 容器，与 devpilot-claude（仅 Claude Code）通过 docker network 解耦，可独立扩缩与重建。
2. **devpilot-litellm 通过 pip 固定版本**：LiteLLM 是 PyPI 包，通过 `pip install litellm==<version>` 安装到 `python:3.13-slim` 基础镜像；版本通过 build-arg 参数化（在 `versions.env` 的 `LITELLM_VERSION` 中维护）。
...
```
1. 启动 Claude Code（无独立代理进程；路由由 devpilot-litellm 通过 docker network 提供）
...
| devpilot-litellm | 512MB | 256MB | 50% | LiteLLM 代理（docker-compose limits 512M / reservations 256M） |
...
- **非 root 运行**：devpilot-claude 与 devpilot-litellm 容器均在 Dockerfile 中创建专用用户并 chown 持久化目录，仅需 `USER <user>` 切换执行用户即可。
...
| devpilot-litellm | 4000 | 127.0.0.1:4000 | HTTP | 端口映射（仅回环） | LiteLLM 代理，Claude Code 经 docker network 访问 |
...
| devpilot-litellm | 1.82.6 | `versions.env` -> `LITELLM_VERSION` | Claude Code ↔ 各平台 OpenAI 翻译 |
...
| devpilot-litellm | HTTP | `curl http://127.0.0.1:4000/health/liveliness` | 30s | 10s | 5 | 20s |
