...
| devpilot-claude-litellm | LiteLLM 代理 | Anthropic↔OpenAI 协议翻译代理，Claude Code 经 http://127.0.0.1:4000 访问各平台 |
| Claude Code | - | Anthropic 出品的终端 AI 编程助手 |
| agnes-ai | Agnes AI | 大模型 API 服务平台（国内 .cn 域名，提供 OpenAI 兼容接口），DevPilot 默认供应商；亦支持 deepseek/glm/ark/bailian |
...
| 能力域 | 能力描述 | 承载组件 |
|--------|---------|---------|
| AI 对话服务 | 飞书机器人接收用户消息，调用配置的大模型（支持 agnes/deepseek/glm/ark/bailian）生成回复 | OpenClaw + Redis |
| AI 编程服务 | 通过 Claude Code 进行代码生成、重构、调试 | Claude Code + 内置 LiteLLM |
| 供应商管理 | 管理 AI 模型供应商配置，支持多供应商切换 | devpilot-claude-litellm |
...
DevPilot 支持 5 个主流大模型平台，通过 `LLM_PLATFORM` 环境变量选择，所有平台均走 **OpenAI Chat Completion 协议**。Agnes AI 为国内（`.cn` 域名）大模型服务平台，其余平台亦均为 OpenAI 兼容接口，可无缝切换。
...
> 切换平台只需修改 `.env` 中的 `LLM_PLATFORM` 与对应平台的 API Key，重启服务即可生效。配置向导（`init.sh`）、部署脚本（`deploy.sh`）、本地运行脚本（`docker-run.sh`）与 devpilot-claude-litellm 启动脚本（`start.sh`）均已内置 5 平台的分支逻辑。
...
4. **资源高效利用**：三容器架构总内存约 2.5GB（redis 512M + openclaw 1024M + devpilot-claude-litellm 1024M），在普通开发机上即可运行全部功能。
...
graph TB
    ...
    subgraph DevPilot 平台
        subgraph 容器网络 devpilot-network
            ...
            CC[Claude Code + 内置 LiteLLM 容器<br/>编程助手<br/>协议翻译代理]
        end
...
graph LR
    subgraph 依赖关系
        ...
        OC --> CC[Claude Code + 内置 LiteLLM<br/>编程助手]
    end
...
- **devpilot-claude-litellm 依赖 OpenClaw**：devpilot-claude-litellm 容器通过 `depends_on: condition: service_started` 依赖 OpenClaw，确保网络可达。devpilot-claude-litellm 可以通过 OpenClaw Gateway 进行 AI 模型路由，也可以直连 agnes-ai API（fallback 模式）。
...
graph TB
    ...
    subgraph K8s 部署
        ...
        HM --> D3[Deployment: devpilot-claude-litellm]
        D1 -.-> PVC1[PVC: redis-data]
        D2 -.-> PVC2[PVC: openclaw-data]
        D3 -.-> PVC3[PVC: workspace]    D3 -.-> PVC4[PVC: claude-config]
    end
```
...
graph LR
    subgraph 宿主机网络
        H[宿主机]
        H -->|18789| OC_PORT[OpenClaw Gateway]    H -->|4000| LITELLM_PORT[LiteLLM 代理 /v1/messages]
    end
    ...
    subgraph Docker 网络: devpilot-network
        subgraph bridge 驱动
            ...
            CC[devpilot-claude-litellm<br/>:4000 代理（回环）]
        end
    end
...
1. **统一 bridge 网络**：所有容器加入 `devpilot-network` bridge 网络，通过容器名（`redis`、`openclaw`、`devpilot-claude-litellm`）进行 DNS 解析和内部通信。
2. **最小端口暴露**：仅暴露 OpenClaw Gateway（18789）和 LiteLLM 代理（容器内 4000、不映射宿主）两个端口到宿主机，Redis 不暴露。
...
- **AI 模型调度**：将用户消息路由到配置的大模型（按 LLM_PLATFORM，默认 agnes-ai），获取 AI 回复。
- **Gateway API**：提供 HTTP 网关接口，供其他容器（如 devpilot-claude-litellm）调用 AI 能力。
...
| 配置项 | 值 | 说明 |
|--------|---|------|
| 基础镜像 | `node:24.17.0-bookworm` | Node.js 22 LTS，Debian Bookworm |
...
| 资源限制 | 1024MB（V8 堆上限 512MB，`NODE_OPTIONS=--max-old-space-size=512`） | 2026-08-26 由 768MB 上调，修复 gateway V8 堆 OOM |
...
1. 检查 `/data/openclaw/.openclaw/openclaw.json` 是否存在。
...
- **被依赖**：devpilot-claude-litellm 容器（`condition: service_started`）--devpilot-claude-litellm 在 OpenClaw 启动后即可启动，无需等待健康检查。

---

### 4.3 Claude Code + LiteLLM 合体容器

#### 4.3.1 职责

`devpilot-claude-litellm` 是一个合体容器，将 **Claude Code** 与 **LiteLLM 代理** 合并部署在单个容器内（取代了原先独立的 `devpilot-claude` 与 `devpilot-litellm` 两个容器），承担两个角色：

- **LiteLLM 代理**：容器内复刻 CC-Switch v3.16.1「Local Route / Claude Route」能力，对外暴露 `http://127.0.0.1:4000/v1/messages`，让 Claude Code 与全 5 平台均能正常对话。Claude Code 经同容器 `http://127.0.0.1:4000` 访问，无需跨容器网络。
...
| 配置项 | 值 | 说明 |
|--------|---|------|
| 基础镜像 | `node:24.17.0-bookworm` | 与 OpenClaw 统一基础镜像 |
| 安装组件 | `@anthropic-ai/claude-code@2.1.241` + devpilot-claude-litellm (LiteLLM 1.82.6) | - |
| 容器名 | `devpilot-claude-litellm` | 固定名称 |
| 重启策略 | `unless-stopped` | - |
| 工作目录 | `/workspace` | 代码工作区 |
| Home 目录 | `/home/node` | Claude Code 配置存储位置 |
| 工作区卷 | `./workspace:/workspace` | 开发代码持久化 |
| 数据卷 | `./data/devpilot-claude:/home/node` | Claude Code 配置持久化 |
| 日志卷 | `./logs/devpilot-claude:/logs` | - |
| 端口映射 | `127.0.0.1:4000`（仅宿主机回环；容器内经 127.0.0.1 访问） | LiteLLM 代理端口 |
| 资源限制 | 1024MB（Claude Code + LiteLLM 合体，较原 768MB 上调） | - |
...
# LiteLLM 以 python3 venv 安装到合并镜像内（基础镜像为 node:24-bookworm，见 dockerfiles/claude/Dockerfile），不再单独构建 python:3.13-slim 镜像。
...
1. **Claude Code 与 LiteLLM 合体部署**：合并为单个 `devpilot-claude-litellm` 容器，LiteLLM 以进程方式在同容器内后台运行，Claude Code 经 `127.0.0.1:4000` 本地访问，减少容器数量与跨容器网络依赖（原架构为 `devpilot-claude` + `devpilot-litellm` 两个独立容器）。
2. **LiteLLM 通过 pip 固定版本**：LiteLLM 是 PyPI 包，通过 `pip install litellm==<version>` 安装到合并镜像内的 `python3-venv` 虚拟环境（路径 `/opt/litellm/venv`）；版本通过 build-arg 参数化（在 `versions.env` 的 `LITELLM_VERSION` 中维护）。
...
```
1. 启动 Claude Code（无独立代理进程；路由由 devpilot-claude-litellm 通过 docker network 提供）
2. 检查 /home/node/.claude/settings.json 是否存在（首启 init 自动生成）
   -> 不存在：自动写入对应平台（按 LLM_PLATFORM）的供应商配置（首次初始化）
   -> 已存在：跳过，保留用户自定义配置
3. 读取并显示 Web 访问密码
4. 若 DEVPILOT_AUTO_DEPLOY=true，执行 post-dev-hook.sh
5. 打印使用说明
6. tail -f /dev/null 保持容器前台运行
```

**自动配置设计决策**：devpilot-claude-litellm 供应商配置仅在首次启动时写入，后续重启不覆盖。这保证了用户通过 Web UI 修改的配置不会因容器重启而丢失。自动配置内容从环境变量（`AGNES_BASE_URL`、`AGNES_API_KEY`、`ANTHROPIC_MODEL`）读取，实现零手动配置。
...
- **被依赖**：无。devpilot-claude-litellm 是启动链的末端。
...
graph TB
    ...
    subgraph 日志流
        ...
        CL[Claude Code 日志] --> LV3[logs/devpilot-claude/]
    end
```
...
| OpenClaw 配置 | `data/openclaw/.openclaw/openclaw.json` | 文件持久化 | 首次启动从模板生成，后续保留用户修改 |
...
DevPilot 共有 30+ 个环境变量（5 平台 × 多变量 + 通用配置），按使用方式分为三类：
...
| 变量名 | 默认值 | 说明 |
|--------|-------|------|
| `AGNES_BASE_URL` | `https://api.agnes-ai.cn/v1` | agnes-ai API 地址 |
| `AGNES_MODEL` | `agnes-2.5-flash` | 默认模型名称 |
| `OPENCLAW_GATEWAY_PORT` | `18789` | OpenClaw Gateway 端口 |
| `TZ` | `Asia/Shanghai` | 时区 |
| `DEVPILOT_AUTO_DEPLOY` | `false` | 是否启用自动部署 |
| `NODE_IMAGE_TAG` | `24.17.0-bookworm` | Node.js 镜像版本 |
...
graph LR
    ...
    DC -->|build.args| DF2[devpilot-claude-litellm Dockerfile]
    VE --> IS2[deploy.sh]
    IS2 --> ENV
```
...
```ini
# versions.env
NODE_IMAGE_TAG=24.17.0-bookworm
...
**启动依赖链**：Redis（healthy）-> OpenClaw（started）-> devpilot-claude-litellm（started）
...
|---------|---------|---------|------|
| 默认 / full | Redis + OpenClaw + devpilot-claude-litellm | 完整功能 | `docker compose up -d` |
| bot | Redis + OpenClaw | 仅飞书 AI 机器人 | `docker compose --profile bot up -d` |
| dev | Claude Code + 内置 LiteLLM | 仅编程助手 | `docker compose --profile dev up -d` |
...
2. **手动构建镜像**：使用 `docker build` 逐个构建 OpenClaw 和 devpilot-claude-litellm 镜像，同时打 `latest` 和版本号标签。
...
| K8s 资源 | 数量 | 说明 |
|---------|------|------|
| Deployment | 3 | Redis、OpenClaw、devpilot-claude-litellm 各一个 |
| Service | 3 | 每个 Deployment 对应一个 Service |
| ConfigMap | 1 | 非敏感配置（时区、端口、URL 等） |
| Secret | 1 | 敏感配置（API Key、密码、Token） |
| PVC | 4 | Redis 数据、OpenClaw 数据、workspace、Claude Code 配置 |
| Ingress | 可选 | 域名访问配置 |
...
graph LR
    ...
    CM --> D3[devpilot-claude-litellm Deployment]
    SEC --> D1
    SEC --> D2
    SEC --> D3
```
...
graph LR
    subgraph CI 流水线
        ...
        L --> BC[构建 devpilot-claude-litellm 镜像]
        BO --> SS[Trivy 安全扫描]
        BC --> SS
...
2. **构建阶段**：两个镜像（OpenClaw、devpilot-claude-litellm）并行构建，使用 `docker/build-push-action` + BuildKit 缓存（`cache-from: type=gha`）。
...
| 非 root 用户 | 规划中 | devpilot-claude-litellm 容器已创建 node 用户并 chown，但未切换运行用户 |
...
| 配置文件持久化 | OpenClaw 和 Claude Code 配置通过 volume 持久化 |
...
| OpenClaw | 1GB | 1024MB（2026-08-26 由 768MB 上调） | 0% | gateway V8 堆 OOM（2026-08-26，默认堆上限约 256MB 撞限）；配 `NODE_OPTIONS=--max-old-space-size=512` 需容器 ≥1GB |
| devpilot-claude-litellm | 512MB（合并前） | 1024MB | +100% | Claude Code + LiteLLM 合体双进程，合并时上调 |
| **总计** | - | **2.56GB** | - | 加上 Docker 开销实际需 3-4GB |

**设计决策**：资源限制基于实际运行时内存监控设定，保留约 50% 的余量。Redis 的 512MB 限制与 `maxmemory 512mb` 配置一致，确保 Redis 在 OOM 前先触发 LRU 淘汰而非被 Docker 杀死。OpenClaw 的 V8 堆上限显式配置为 512MB 并与容器限制 1024M 配套（堆 + Node 运行时/外部内存开销），避免长 SSE 流式回复堆增长时撞 V8 默认堆上限（详见《运维操作手册》§13.7）。
...
DevPilot 的 AI 研发流程通过 8 个 OpenClaw 技能实现，技能位于 `skills/<技能名>/SKILL.md`（目录 + YAML frontmatter 的 OpenClaw 标准格式）：

| 技能目录 | 阶段 | 角色 | 门控 |
|---------|------|------|------|
| `explore/` | 需求探索 | 产品经理 / 业务分析师 | G1 |
| `prd/` | 需求分析 | 系统架构师 | G2 |
| `plan/` | 计划拆解 | 技术主管 | G3 |
| `dev/` | 开发执行 | 全栈工程师 | - |
| `review/` | 代码审查 | 质量工程师 | - |
| `test/` | 测试验证 | QA 工程师 | G4 |
| `g5-deploy/` | 提交部署 | DevOps 工程师 | G5 |
| `deploy/` | `/deploy` 命令路由 | 命令解释器 | 变更类命令二次确认 |

> 技能安装到 `data/openclaw/.openclaw/skills/`（OpenClaw managed skills root）。
> G5 技能原名 `deploy`，2026-08-26 更名 `g5-deploy`，把 `/deploy` 斜杠命令让给命令路由技能。
...
| devpilot-claude-litellm 资源限制 | 1GB | 768MB | 降低 23% |
...
> 注：表中 OpenClaw 行（768MB）为 2026-08 中旬的历史优化记录；2026-08-26 因 gateway
> V8 堆 OOM 已回调至 1024MB（见 §10.1 资源限制策略）。
...
| 2 | devpilot-claude-litellm 非 root 运行 | 容器安全提升 | Dockerfile 添加 USER node 指令 |
...
- **镜像预构建**：当前首次部署需要从源码构建两个镜像（OpenClaw + devpilot-claude-litellm），耗时 5-10 分钟。CI 流水线已实现镜像构建和推送，部署阶段只需修改 docker-compose.yml 从 `build` 改为 `image` 拉取预构建镜像即可。
- **非 root 运行**：devpilot-claude-litellm 与 devpilot-claude-litellm 容器均在 Dockerfile 中创建专用用户并 chown 持久化目录，仅需 `USER <user>` 切换执行用户即可。
...
├── conf/                          # 配置文件
│   ├── devpilot-claude-litellm/
│   │   └── start.sh               #   devpilot-claude-litellm 启动脚本
│   ├── openclaw/
...
|------|-----------|-------------------|------|---------|------|
| Redis | 6379 | （不发布） | TCP | 仅 bridge 内服务名 `redis:6379` | bridge 模式经服务名访问；不对外暴露（P0-7 后取消宿主机发布） |
| OpenClaw | 18789 | 18789 | HTTP | 端口映射（所有接口） | Gateway API + /healthz；bridge 网络经映射暴露，局域网浏览器可直连（建议配合防火墙/反代） |
| devpilot-claude-litellm | 4000 | 127.0.0.1:4000 | HTTP | 端口映射（仅回环） | LiteLLM 代理；Claude Code 经同容器 127.0.0.1 访问，OpenClaw 经服务名 `devpilot-claude-litellm:4000` 跨容器访问 |

**端口可配置**：OpenClaw 的宿主机端口通过 `OPENCLAW_GATEWAY_PORT` 环境变量配置；LiteLLM 容器内 4000 仅映射宿主机回环（供脚本健康检查），跨容器访问走服务名。
...
| `AGNES_BASE_URL` | agnes-ai | `https://api.agnes-ai.cn/v1` | 否 | API 地址 |
| `AGNES_MODEL` | agnes-ai | `agnes-2.5-flash` | 否 | 默认模型名称 |
...
| `NODE_IMAGE_TAG` | 版本 | `24.17.0-bookworm` | 否 | Node.js 镜像版本 |
| `OPENCLAW_VERSION` | 版本 | `2026.7.1-2` | 否 | OpenClaw 版本 |
| `CLAUDE_CODE_VERSION` | 版本 | `2.1.241` | 否 | Claude Code 版本 |

### 13.4 版本配置一览

| 组件 | 当前版本 | 配置位置 | 说明 |
|------|---------|---------|------|
| Node.js 基础镜像 | 24.17.0-bookworm | `versions.env` -> `NODE_IMAGE_TAG` | Node.js 22 LTS |
| OpenClaw | 2026.7.1-2 | `versions.env` -> `OPENCLAW_VERSION` | 飞书 AI 机器人框架 |
| devpilot-claude-litellm | 1.82.6 | `versions.env` -> `LITELLM_VERSION` | Claude Code ↔ 各平台 OpenAI 翻译 |
| Claude Code | 2.1.241 | `versions.env` -> `CLAUDE_CODE_VERSION` | AI 编程助手 |
...
| devpilot-claude-litellm | HTTP | `curl http://127.0.0.1:4000/health/liveliness` | 30s | 10s | 5 | 20s |
