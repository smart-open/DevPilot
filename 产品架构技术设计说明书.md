...
> 切换平台只需修改 `.env` 中的 `LLM_PLATFORM` 与对应平台的 API Key，重启服务即可生效。配置向导（`init.sh`）、部署脚本（`deploy.sh`）、本地运行脚本（`docker-run.sh`）与 CC-Switch 启动脚本（`start.sh`）均已内置 5 平台的分支逻辑。
...
| 数据卷 | `./data/devpilot-claude:/home/node` | CC-Switch 配置持久化 |
...
graph TB
    ...
    subgraph 日志流
        ...
        CL[CC-Switch 日志] --> LV3[logs/devpilot-claude/]
    end
```
