...
```yaml
healthcheck:
  # CC-Switch Web 启用登录鉴权，未鉴权访问 / 返回 401 属正常“已就绪”态，故不用 -f（4xx 判失败）
  test: ["CMD", "curl", "-s", "-o", "/dev/null", "http://127.0.0.1:${CC_SWITCH_WEB_PORT}"]
  interval: 30s
  timeout: 10s
  retries: 3
  start_period: 30s
...
| CC-Switch 配置 | `data/devpilot-claude/.cc-switch/` | 文件持久化 | 供应商配置和 Web 密码 |
