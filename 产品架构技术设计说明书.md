...
|------|-----------|-------------------|------|---------|------|
| Redis | 6379 | 127.0.0.1:6379 | TCP | 端口映射（仅回环） | OpenClaw host 模式经 127.0.0.1 访问；不对外暴露 |
| OpenClaw | 18789 | 18789 | HTTP | host 网络 | Gateway API + /healthz（network_mode: host） |
| devpilot-claude-litellm | 4000 | 127.0.0.1:4000 | HTTP | 端口映射（仅回环） | LiteLLM 代理，Claude Code 经 docker network 访问 |

**端口可配置**：OpenClaw 与 LiteLLM 的宿主机端口分别通过 `OPENCLAW_GATEWAY_PORT` 环境变量配置；LiteLLM 默认监听容器内 4000、不映射宿主。
